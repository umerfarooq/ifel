  ...
  module Acts
    module Likeable
      ...
      module InstanceMethods
        ...
        # Helper method to see if a user has any liking for any likeable
        def liking_by_user?(user)
          rtn = false
          if user
            self.likings.each { |l|
        ...
        def is_liked_by_user?(user)
          self.likings.find(:first, :conditions=>{:user_id => user.id, :liking => true})
        end

        def is_disliked_by_user?(user)
          self.likings.find(:first, :conditions=>{:user_id => user.id, :liking => false})
        end
      end
    end
  end
