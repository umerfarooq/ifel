...
def UpdateSubscription(auth, subId, amount, trialamount,creditcardinfo)
...
x.ARBUpdateSubscriptionRequest("xmlns" =>"AnetApi/xml/v1/schema/AnetApiSchema.xsd") {
 x.merchantAuthentication  {
    x.name auth.name
    x.transactionKey auth.transactionKey
  }
  x.subscriptionId subId
  x.subscription {
    x.amount amount
    x.trialAmount trialamount
    x.payment {
...
def CancelSubscription(auth, subId)
...
if root.elements["messages/resultCode"].text != "Ok"
   ProcessErrorResponse root.elements["messages"]
elsif !(root.elements["subscriptionId"].blank?)
   ProcessCreateSubscriptionResponseSuccess root
else
   ProcessSubscriptionResponseSuccess root
end
...
def ProcessSubscriptionResponseSuccess(root)
